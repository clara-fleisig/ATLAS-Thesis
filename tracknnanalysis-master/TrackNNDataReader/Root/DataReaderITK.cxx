...
void DataReaderITK::initialize()
{   
    readGeo();

    // if( indir.substr(indir.size()-1, 1) != "/" )
    // indir += "/";
    ...
    tree = (TTree*) m_inFile->Get("tracksummary_ambiguity");
    m_recoTracks = std::make_shared<RecoTracksReaderClass>(tree);

    m_nEvents               = tree->GetEntries();

    // m_hitsFile = TFile::Open(m_inDir + "hits.root");
    // m_particlesFile = TFile::Open(m_inDir + "fatras_particles_initial.root");
    // m_tracksFile = TFile::Open(m_inDir + "tracksummary_ambiguity.root");

    // auto tree = (TTree*) m_hitsFile->Get("hits");
    // m_hits = std::make_shared<HitReaderClass>(tree);

    // tree = (TTree*) m_particlesFile->Get("particles");
    // m_truthTracks = std::make_shared<TruthTracksReaderClass>(tree);

    // tree = (TTree*) m_tracksFile->Get("tracksummary");
    // m_recoTracks = std::make_shared<RecoTracksReaderClass>(tree);
...
void DataReaderITK::loadEntry(int iEvent)
{
    ...
    else if(m_LayerRegion.Contains("custom"))
    {
        lwrEtaLim = -5;
        uprEtaLim = 5;
        Z0Lim = 150;
    ...
    unsigned int eventID = m_recoTracks->event_nr();
    ...
    for(int i = 0; i < nTracks; i++)
    {
        ...
        if(m_truthTracks->eta(i) < lwrEtaLim) continue;
        if(m_truthTracks->eta(i) > uprEtaLim) continue;
    ...
    for(int i = 0; i < nRecoTracks; i++)
    {
        ...
        // map phi to lie between -pi to pi
        // if (phi > M_PI) phi = phi - 2*M_PI;
        // if (phi < -M_PHI) phi = phi + 2*M_PI;
...
void DataReaderITK::readGeo()
{
    ...
    std::cout<<"reading detector geometry "<<m_geofileName<<std::endl;
...
//Fix This
void DataReaderITK::predictNextVolumeAndLayer(std::shared_ptr<Hit> hit, std::vector<std::pair<int, int>>& nextID)
{
