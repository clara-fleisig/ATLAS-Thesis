...
void extrapolateTracks(vector<shared_ptr<ExtrapolatedTrack>>& completeTracks, vector<shared_ptr<SeedTrack>>& seedTracks, const shared_ptr<HitsManager> hitManager, const shared_ptr<NNEvalCombined> nnEval, 
...
int main(int argc, char *argv[])
{
    ...
    // auto nnClassEval = make_shared<NNEvalClassUtils>(NNFile, atoi(opts["windowSize"].c_str()));
    //nnEval->initialize();

    auto NNCombined = make_shared<NNEvalCombined>(NNClassFile, NNExtrapFile, atoi(opts["windowSize"].c_str()));
    NNCombined->initialize(); 
            ...
            for (const auto& minHit: minNHitsList)
            {
                ...
                for(int i = 0; i < nThreads; i++)
                {
                    ...
                    threads.push_back(std::thread(extrapolateTracks, std::ref(completedTracksMap[i]), std::ref(subSeedMap[i]), 
                        std::ref(hitManager), std::ref(NNCombined), std::ref(dataReader), err, minHit, algoType));
                }
...
const shared_ptr<HitsManager> hitManager, const shared_ptr<NNEvalCombined> nnEval, const shared_ptr<DataReaderITK> dataReader, 
int uncertainty, int minHits, std::string algoType)
{   
    ...
    {   
        ...
        while (roads.size() > 0) // after a road is complete, it should be stored in another container and removed from roads
        {
            ...
            for(auto& trk: roads)
            {
                ...
                if (trk->getHitsList().at(0)->getTruthTrackBarcode().at(0) == 63050394883850240)
                    trk->printDebug();

                //cout << "---- New Roads Being Built ----" << std::endl;
                auto hitList = trk->getHitsList();
                //std::shared_ptr<Hit> predictedHit = nullptr;
                ...
                if (algoType == "slidingWindow")
                {   //predictedHit = nnEval->ClassifyAndExtrapolate(hitList, false);
                    auto predictedHitPair = nnEval->Extrap(hitList, false);
                    predictedHitPairs.push_back(predictedHitPair);
                    //predictedHitPairs = nnEval->ExtrapMultiple(hitList, 0.3,false);
                }
                //std::cout<<"n predictions "<<predictedHitPairs.size()<<std::endl;
                // else if (algoType == "fullTrack")
                //     predictedHit = nnEval->extrapolate(hitList, trk->getNHits()-1, false); 

                //auto recoveredHits = hitManager->getClosestHitsListFromPredictedHit(predictedHit, hitList, uncertainty);
                for (auto& pr: predictedHitPairs)
                {
                    ...
                    auto recoveredHits = hitManager->getClosestLayerHitsList(pr.first, hitList, volumeId, layerId, uncertainty);
